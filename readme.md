...
- **🔒 Concurrency-Safe** — Serieller Write-Lock und Stateful-Sessions mit automatischer Lock-Recovery verhindern Konflikte bei parallelen Schreiboperationen. Der ADT-Client-Layer ist als **Session-Pool** ausgelegt (`registerSession`/`getClientFor`): lokal/stdio über eine implizite Default-Session, optional pro Nutzer isolierte Sessions als Fundament für einen gehosteten Mehrbenutzer-Betrieb (HTTP/Cloud Foundry).
